# Pre ADSI (Addressed Exchange 2013 remoting issues)
        ...
        foreach ($Mailbox in $MailboxList) {
                ...
                Foreach ($CalAccess in $CalAccessList) {
                    ...
                    if ($GroupMemberHash["$Logon"].Members -and $ADHashDisplay["$DisplayType"] -match 'group') {
                        foreach ($Member in @($GroupMemberHash.$Logon.Members)) {
                            Write-Verbose "Granted calendar group member`t$Member"
                    ...
                    elseif ( $ADHashDisplayName[$("$CalAccess.User")].objectClass -notmatch 'group') {
                        Write-Host "Granted calendar`t$($CalAccess.User)" -ForegroundColor Green
                        New-Object -TypeName psobject -property @{
                            Object             = $Mailbox.DisplayName
                            UserPrincipalName  = $Mailbox.UserPrincipalName
                            PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
            ...
            If ($InboxAccessList) {
                Foreach ($InboxAccess in $InboxAccessList) {
                    ...
                    if ($GroupMemberHash["$Logon"].Members -and $ADHashDisplay["$DisplayType"] -match 'group') {
                        foreach ($Member in @($GroupMemberHash.$Logon.Members)) {
                            Write-Verbose "Granted inbox group member`t$Member"
                            New-Object -TypeName psobject -property @{
                                Object             = $Mailbox.DisplayName
                                UserPrincipalName  = $Mailbox.UserPrincipalName
                                PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
                    ...
                    elseif ( $ADHashDisplayName[$("$InboxAccess.User")].objectClass -notmatch 'group') {
                        Write-Host "Granted inbox user`t$($InboxAccess.User)" -ForegroundColor Green
                        New-Object -TypeName psobject -property @{
                            Object             = $Mailbox.DisplayName
                            UserPrincipalName  = $Mailbox.UserPrincipalName
                            PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
            ...
            If ($SentAccessList) {
                Foreach ($SentAccess in $SentAccessList) {
                    ...
                    if ($GroupMemberHash["$Logon"].Members -and $ADHashDisplay["$DisplayType"] -match 'group') {
                        foreach ($Member in @($GroupMemberHash.$Logon.Members)) {
                            Write-Verbose "Granted sentitems group member`t$Member"
                            New-Object -TypeName psobject -property @{
                                Object             = $Mailbox.DisplayName
                                UserPrincipalName  = $Mailbox.UserPrincipalName
                                PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
                    ...
                    elseif ( $ADHashDisplayName[$("$SentAccess.User")].objectClass -notmatch 'group') {
                        Write-Host "Granted sentitems user`t$($SentAccess.User)" -ForegroundColor Green
                        New-Object -TypeName psobject -property @{
                            Object             = $Mailbox.DisplayName
                            UserPrincipalName  = $Mailbox.UserPrincipalName
                            PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
            ...
            If ($ContactsAccessList) {
                Foreach ($ContactsAccess in $ContactsAccessList) {
                    ...
                    if ($GroupMemberHash["$Logon"].Members -and $ADHashDisplay["$DisplayType"] -match 'group') {
                        foreach ($Member in @($GroupMemberHash.$Logon.Members)) {
                            Write-Verbose "Granted contacts group member`t$Member"
                            New-Object -TypeName psobject -property @{
                                Object             = $Mailbox.DisplayName
                                UserPrincipalName  = $Mailbox.UserPrincipalName
                                PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
                    ...
                    elseif ( $ADHashDisplayName[$("$ContactsAccess.User")].objectClass -notmatch 'group') {
                        Write-Host "Granted contacts user`t$($ContactsAccess.User)" -ForegroundColor Green
                        New-Object -TypeName psobject -property @{
                            Object             = $Mailbox.DisplayName
                            UserPrincipalName  = $Mailbox.UserPrincipalName
                            PrimarySMTPAddress = $Mailbox.PrimarySMTPAddress
...
Get-Answer
# Get-MailboxMoveOnPremisesPermissionReport -ReportPath ([Environment]::GetFolderPath("Desktop")) -Verbose -skipsendas -SkipFullAccess -SkipSendOnBehalf
