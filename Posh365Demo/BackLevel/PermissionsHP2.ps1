...
function Get-MailboxMoveOnPremisesPermissionReport {
    ...
    param (
        ...
        if (-not $ADGroups) {
            $Global:ADGroups = Get-ADGroup @GroupParams
        }
        if (-not $UserGroupHash -and -not $ADUserList) {
            $Global:ADUserList = Get-ADUsersAndGroups -DomainNameHash $DomainNameHash -ADGroups $ADGroups
